...
monthly_loss_waste <- 
  ggplot(data = df, aes(x = day_name, y = food_loss_kg + food_waste_kg, group=1)) +
  geom_line(color="dark blue") +
  geom_point(aes(shape = is_closed)) +
  scale_shape_manual(values=c(16, 4)) +
...
monthly_solid_waste <- 
  ...
  theme(legend.position = "none") +
  xlab("Date") +
  ylab("Monthly Solid Food Waste (kg)") +
...
tsPlot_total_orders <- 
  ...
  scale_color_manual(name='Packages',
                     ...
                     values=c('fulls'='dark blue',
                              'halfs'='purple', 'takeouts'='dark red'))+
...
tsPlot_dineinKg <- 
  ggplot() +
  geom_line(data = subset(df, is_closed %in% FALSE), 
            aes(x = date, y = daily_total_served), color="dark blue") +
  geom_line(data = subset(df, is_closed %in% FALSE), 
            aes(x = date, y = liquors), color="green") +
  theme(legend.position = "right") +
  xlab("Date") +
  ylab("Daily Dine-in Served in Kilogram") +
  ggtitle("Daily Dine-in Served Plot")
tsPlot_dineinKg
...
tsPlot_sales <- 
  ggplot() +
  geom_line(data = subset(df, is_closed %in% FALSE),
            aes(x = date, y = sales), color="purple") +
  stat_smooth(data = subset(df, is_closed %in% FALSE),
              aes(x = date, y = sales),
              color = "#00AFBB", fill = "#00AFBB",method = "loess") +
  xlab("Date") +
  ylab("Daily Sales in dollar") +
  ggtitle("Daily Sales Plot")
tsPlot_sales

grid.arrange(tsPlot_temp,tsPlot_humidity, tsPlot_precip)
```
