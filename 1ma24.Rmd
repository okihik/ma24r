...
hist_food_waste_wk <- 
  ggplot(data = subset(df %>% 
                         mutate(wkend = if_else(wday(date) %in%
                                                  c(1,6,7),T,F)), 
                       is_closed %in% FALSE), 
         aes(x = food_waste_kg,  colour = factor(wkend))) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, fill = "white") +
  geom_density(linewidth = 1.5, fill = 4, alpha = 0.15) +
  labs(title = "Food Waste - Histogram")
hist_food_waste_wk
...
hist_solid_waste_wk <- 
  ggplot(data = subset(df%>% 
                         mutate(wkend = if_else(wday(date) %in%
                                                  c(1,6,7),T,F)), 
                       is_closed %in% FALSE),
         aes(x = solid_waste_kg, colour = factor(wkend))) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = 30, fill = "white") +
  geom_density(linewidth = 1.5, fill = 4, alpha = 0.15) +
  labs(title = "Solid Food Waste - Histogram")
...
hist_liquid_waste_wk <- 
  ggplot(data = subset(df %>% 
                         mutate(wkend = if_else(wday(date) %in%
                                                  c(1,6,7),T,F)),
                       is_closed %in% FALSE), 
         aes(x = liquid_waste_kg, colour = factor(wkend))) +
  geom_histogram(aes(y = after_stat(density)), 
                 bins = 30, fill = "white") +
  geom_density(linewidth = 1.5, fill = 4, alpha = 0.15) +
  labs(title = "Liquid Food Waste - Histogram")
