---
title: "Log-linear RDiT - MA Data Analysis"
author: "Akihiko Mori"
date: "02/02/2024"
output: 
...
## No Interaction
```{r rdint log-linear model no interaction}
# Interaction
## simple food waste per customer -----
lm_fw_log <- log_food_waste_kg ~ D_01 + t_01 +
...
lm_sfw_log <- log_solid_waste_kg ~ container + time  +
                                      temp_c + humi_p + prcp_mm + 
                                      liquors + sales + halfs +
                                      tueE + wedE+ thuE+ friE+ satE
lm_sfw_log <- df %>% 
...
lm_lfw_log <- log_liquid_waste_kg ~ container + time  +
                                      temp_c + humi_p + prcp_mm + 
                                      liquors + sales + halfs +
                                      tueE + wedE+ thuE+ friE+ satE
lm_lfw_log <- df %>% 
...
lm_mult_of <- list(
  "lm food waste"   = lm(lm_fw_log,  
                         data = subset(df, df$is_closed == FALSE)),
  "lm solid waste"  = lm(lm_fw_log, 
                         data = subset(df, df$is_closed == FALSE)),
  "lm liquid waste" = lm(lm_fw_log, 
                         data = subset(df, df$is_closed == FALSE))
)
```
...
## Multiple model ----
# multi food waste -----
rdt_fw <- log_food_waste_kg ~ D_01 * t_01 +
                            temp_c + humi_p + prcp_mm + 
                            tueE + wedE+ thuE+ friE+ satE+
                            liquors + sales + halfs
rdt_fw <- df %>% 
...
rdt_sfw <- log_solid_waste_kg ~ D_01 * t_01 +
                            temp_c + humi_p + prcp_mm + 
                            tueE + wedE+ thuE+ friE+ satE+
                            liquors + sales + halfs
rdt_sfw <- df %>% 
...
rdt_lfw <- log_liquid_waste_kg ~ D_01 * t_01 +
                            temp_c + humi_p + prcp_mm + 
                            tueE + wedE+ thuE+ friE+ satE+
                            liquors + sales + halfs
rdt_lfw <- df %>% 
...
rdt_of <- list(
  "log food waste"= lm(rdt_fw,  
                         data = subset(df, df$is_closed == FALSE)),
  "food waste"    = lm(rdt_fw,  
                         data = subset(df, df$is_closed == FALSE)),
  "log solid waste" = lm(rdt_sfw, 
                         data = subset(df, df$is_closed == FALSE)),
  "solid waste"  = lm(rdt_sfw, 
                         data = subset(df, df$is_closed == FALSE)),
  "log liquid waste" = lm(rdt_lfw, 
                         data = subset(df, df$is_closed == FALSE)),
  "liquid waste" = lm(rdt_lfw, 
                         data = subset(df, df$is_closed == FALSE))
)
cm <- c('D_01' = 'D',
        'D_01:t_01' = 'D × t',
...
library(performance)
ass_multi_fw_log  <- plot(check_model(rdt_fw,  
                                      detrend=FALSE,
                                      panel = FALSE,
                                      colors = c("#3aaf85","black","#cd201f")))
ass_multi_sfw_log <- plot(check_model(rdt_sfw, 
                                      detrend=FALSE,
                                      panel = FALSE,
                                      colors = c("#3aaf85","dark orange","#cd201f")))
ass_multi_lfw_log <- plot(check_model(rdt_lfw, 
                                      detrend=FALSE,
                                      panel = FALSE,
...
check_autocorrelation(rdt_of$`food waste`)
check_autocorrelation(rdt_of$`solid waste`)
check_autocorrelation(rdt_of$`liquid waste`)

# 1.2 perform Durbin-Watson test
durbinWatsonTest(rdt_of$`food waste`)
durbinWatsonTest(rdt_of$`solid waste`)
durbinWatsonTest(rdt_of$`liquid waste`)  
dwtest(rdt_of$`food waste`)
dwtest(rdt_of$`solid waste`)
dwtest(rdt_of$`liquid waste`)   


# 2. Normality of the residuals -----------
# 2.1 histogram of residuals
# Normality of Residuals: Food Waste
ass_norm_fw <- plot(check_normality(rdt_of$`food waste`), 
                    type = "density") + 
  labs(title = "Normality of Residuals: Food Waste", subtitle = "")
# Normality of Residuals: Solid Food Waste
ass_norm_sfw <- plot(check_normality(rdt_of$`solid waste`), 
                    type = "density") + 
  labs(title = "Normality of Residuals: Solid Food Waste", subtitle = "")
# Normality of Residuals: Liquid Food Waste
ass_norm_lfw <- plot(check_normality(rdt_of$`liquid waste`), 
                    type = "density") + 
  labs(title = "Normality of Residuals: Liquid Food Waste", subtitle = "")
ass_norm_fw;ass_norm_sfw;ass_norm_lfw


# 2.2 QQ plots of Residuals
ass_qq_fw <- ass_multi_fw_log[[6]]  + 
  labs(title = "QQ Plot of Residuals: Food Waste", subtitle = "")
ass_qq_sfw <- ass_multi_sfw_log[[6]] + 
  labs(title = "QQ Plot of Residuals: Solid Food Waste", subtitle = "")
ass_qq_lfw <- ass_multi_lfw_log[[6]] + 
  labs(title = "QQ Plot of Residuals: Liquid Food Waste", subtitle = "")
ass_qq_fw;ass_qq_sfw;ass_qq_lfw

# 2.3 shapiro-wilk normality test
check_normality(rdt_of$`food waste`)
check_normality(rdt_of$`solid waste`)
check_normality(rdt_of$`liquid waste`)


# 3. No influential points (outliers) --------
ass_out_fw  <- ass_multi_fw_log[[4]]  + 
  labs(title = "Outliers: Food Waste", subtitle = "")
ass_out_sfw <- ass_multi_sfw_log[[4]] + 
  labs(title = "Outliers: Solid Food Waste", subtitle = "")
ass_out_lfw <- ass_multi_lfw_log[[4]] + 
  labs(title = "Outliers: Liquid Food Waste", subtitle = "")
ass_out_fw;ass_out_sfw;ass_out_lfw

# 3.1 Check outliers
check_outliers(rdt_of$`food waste`)
check_outliers(rdt_of$`solid waste`)
check_outliers(rdt_of$`liquid waste`)

# 4. Homoscedasticity of the residuals -----
# 4.1 plot residuals
ass_homo_fw  <- ass_multi_fw_log[[3]]  + 
  labs(title = "Homoscedasticity: Food Waste", subtitle = "")
ass_homo_sfw <- ass_multi_sfw_log[[3]] + 
  labs(title = "Homoscedasticity: Solid Food Waste", subtitle = "")
ass_homo_lfw <- ass_multi_lfw_log[[3]] + 
  labs(title = "Homoscedasticity: Liquid Food Waste", subtitle = "")
ass_homo_fw;ass_homo_sfw;ass_homo_lfw


# 4.2 Breusch-Pagan test
# to test for heteroskedasticity in a linear regression model.
# 
lmtest::bptest(rdt_of$`food waste`)
lmtest::bptest(rdt_of$`solid waste`)
lmtest::bptest(rdt_of$`liquid waste`)

# 5. Linearity of the relationships b/w the dependent and independent variables ------
# 5.1 plot residual vs fitted values
ass_ln_fw  <- ass_multi_fw_log[[2]]  + 
  labs(title = "Linearity: Food Waste", subtitle = "")
ass_ln_sfw <- ass_multi_sfw_log[[2]] + 
  labs(title = "Linearity: Solid Food Waste", subtitle = "")
ass_ln_lfw <- ass_multi_lfw_log[[2]] + 
  labs(title = "Linearity: Liquid Food Waste", subtitle = "")
ass_ln_fw;ass_ln_sfw;ass_ln_lfw


# 5.2 check linearity
check_heteroscedasticity(rdt_of$`food waste`)
check_heteroscedasticity(rdt_of$`solid waste`)
check_heteroscedasticity(rdt_of$`liquid waste`)

# 6. No multicollinearity
ass_multi_sfw_log[[5]]  + labs(title = "VIF", subtitle = "")
#6.1 check collinearity
coll_fw  <- check_collinearity(rdt_of$`food waste`)
coll_fw
```
```{r eval = FALSE, echo = FALSE}
...
comp_rdt_of <-  list(
    "RDiT: food waste"   = lm(rdt_fw,  
                            data = subset(df, df$is_closed == FALSE)),
    "MANOVA: food waste" = lm(rdt_fw_log,
                              data = subset(df, df$is_closed == FALSE)),
    "RDiT: solid waste"  = lm(rdt_sfw, 
                            data = subset(df, df$is_closed == FALSE)),
    "MANOVA: solid waste"  = lm(rdt_sfw_log, 
                                data = subset(df, df$is_closed == FALSE)),
    "RDiT: liquid waste" = lm(rdt_lfw, 
                            data = subset(df, df$is_closed == FALSE)),
    "MANOVA: liquid waste" = lm(rdt_lfw_log,
                                data = subset(df, df$is_closed == FALSE))
  )
...
comp_rdt<- modelsummary(comp_rdt_of,
             estimate = "{estimate}{stars}",
             statistic = c("s.e. = {std.error}"),
             stars = c('*' = .05),
...
cont_cov <- ggarrange(continuity_temp,continuity_humidity,
                     continuity_precip,continuity_liquor,
                     continuity_halfs,continuity_sales, 
                     ncol=2, nrow=3, 
                     common.legend = TRUE, legend = "bottom",
                     legend.grob  = )
cont_cov
...
rdt_fw_11 <- log_food_waste_kg ~ D_11 * t_11 + 
                                 temp_c + humi_p + prcp_mm + 
                                 liquors + sales + halfs +
                                 tueE + wedE+ thuE+ friE+ satE
rdt_sfw_11 <- log_solid_waste_kg ~ D_11 * t_11 + 
                                   temp_c + humi_p + prcp_mm + 
                                   liquors + sales + halfs +
                                   tueE + wedE+ thuE+ friE+ satE
rdt_lfw_11 <- log_liquid_waste_kg ~ D_11 * t_11 + 
                                    temp_c + humi_p + prcp_mm + 
                                    liquors + sales + halfs +
                                    tueE + wedE+ thuE+ friE+ satE

# formulas: cutoff 12/01
rdt_fw_12 <- log_food_waste_kg ~ D_12 * t_12 + 
                                 temp_c + humi_p + prcp_mm + 
                                 liquors + sales + halfs +
                                 tueE + wedE+ thuE+ friE+ satE
rdt_sfw_12 <- log_solid_waste_kg ~ D_12 * t_12 + 
                                   temp_c + humi_p + prcp_mm + 
                                   liquors + sales + halfs +
                                   tueE + wedE+ thuE+ friE+ satE
rdt_lfw_12 <- log_liquid_waste_kg ~ D_12 * t_12 + 
                                    temp_c + humi_p + prcp_mm + 
                                    liquors + sales + halfs +
                                    tueE + wedE+ thuE+ friE+ satE
# formulas: cutoff 2/01
rdt_fw_02 <- log_food_waste_kg ~ D_02 * t_02 + 
                                 temp_c + humi_p + prcp_mm + 
                                 liquors + sales + halfs +
                                 tueE + wedE+ thuE+ friE+ satE
rdt_sfw_02 <- log_solid_waste_kg ~ D_02 * t_02 + 
                                   temp_c + humi_p + prcp_mm + 
                                   liquors + sales + halfs +
                                   tueE + wedE+ thuE+ friE+ satE
rdt_lfw_02 <- log_liquid_waste_kg ~ D_02 * t_02 + 
                                    temp_c + humi_p + prcp_mm + 
                                    liquors + sales + halfs +
                                    tueE + wedE+ thuE+ friE+ satE
# formulas: cutoff 3/01
rdt_fw_03 <- log_food_waste_kg ~ D_03 * t_03 + 
                                 temp_c + humi_p + prcp_mm + 
                                 liquors + sales + halfs +
                                 tueE + wedE+ thuE+ friE+ satE
rdt_sfw_03 <- log_solid_waste_kg ~ D_03 * t_03 + 
                                   temp_c + humi_p + prcp_mm + 
                                   liquors + sales + halfs +
                                   tueE + wedE+ thuE+ friE+ satE
rdt_lfw_03 <- log_liquid_waste_kg ~ D_03 * t_03 + 
                                    temp_c + humi_p + prcp_mm + 
                                    liquors + sales + halfs +
                                    tueE + wedE+ thuE+ friE+ satE
# perform linear regression -------------
plac_fw_d <- list(
  "cutoff_1101" = lm(rdt_fw_11, data = subset(df, !is_closed)),
  "cutoff_1203" = lm(rdt_fw_12, data = subset(df, !is_closed)),
  "cutoff_0103" = lm(rdt_fw,    data = subset(df, !is_closed)),
  "cutoff_0201" = lm(rdt_fw_02, data = subset(df, !is_closed)),
  "cutoff_0301" = lm(rdt_fw_03, data = subset(df, !is_closed))
)
plac_sfw_d <- list(
  "cutoff_1101" = lm(rdt_sfw_11, data = subset(df, !is_closed)),
  "cutoff_1203" = lm(rdt_sfw_12, data = subset(df, !is_closed)),
  "cutoff_0103" = lm(rdt_sfw,    data = subset(df, !is_closed)),
  "cutoff_0201" = lm(rdt_sfw_02, data = subset(df, !is_closed)),
  "cutoff_0301" = lm(rdt_sfw_03, data = subset(df, !is_closed))
)
plac_lfw_d <- list(
  "cutoff_1101" = lm(rdt_lfw_11, data = subset(df, !is_closed)),
  "cutoff_1203" = lm(rdt_lfw_12, data = subset(df, !is_closed)),
  "cutoff_0103" = lm(rdt_lfw,    data = subset(df, !is_closed)),
  "cutoff_0201" = lm(rdt_lfw_02, data = subset(df, !is_closed)),
  "cutoff_0301" = lm(rdt_lfw_03, data = subset(df, !is_closed))
)
