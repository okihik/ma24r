...
rdt_of <- list(
  "rdt food waste"   = lm(rdt_fw,  
                         data = subset(df, df$is_closed == FALSE)),
  "rdt solid waste"  = lm(rdt_sfw, 
                         data = subset(df, df$is_closed == FALSE)),
  "rdt liquid waste" = lm(rdt_lfw, 
                         data = subset(df, df$is_closed == FALSE))
)
```
...
check_autocorrelation(rdt_of$`rdt food waste`)
check_autocorrelation(rdt_of$`rdt solid waste`)
check_autocorrelation(rdt_of$`rdt liquid waste`)

# 1.2 perform Durbin-Watson test
durbinWatsonTest(rdt_of$`rdt food waste`)
durbinWatsonTest(rdt_of$`rdt solid waste`)
durbinWatsonTest(rdt_of$`rdt liquid waste`)  
dwtest(rdt_of$`rdt food waste`)
dwtest(rdt_of$`rdt solid waste`)
dwtest(rdt_of$`rdt liquid waste`)   


# 2. Normality of the residuals -----------
# 2.1 histogram of residuals
# Normality of Residuals: Food Waste
ass_norm_fw <- plot(check_normality(rdt_of$`rdt food waste`), 
                    type = "density") + 
  labs(title = "Normality of Residuals: Food Waste", subtitle = "")
# Normality of Residuals: Solid Food Waste
ass_norm_sfw <- plot(check_normality(rdt_of$`rdt solid waste`), 
                    type = "density") + 
  labs(title = "Normality of Residuals: Solid Food Waste", subtitle = "")
# Normality of Residuals: Liquid Food Waste
ass_norm_lfw <- plot(check_normality(rdt_of$`rdt liquid waste`), 
                    type = "density") + 
  labs(title = "Normality of Residuals: Liquid Food Waste", subtitle = "")
...
# 2.3 shapiro-wilk normality test
check_normality(rdt_of$`rdt food waste`)
check_normality(rdt_of$`rdt solid waste`)
check_normality(rdt_of$`rdt liquid waste`)
...
# 3.1 Check outliers
check_outliers(rdt_of$`rdt food waste`)
check_outliers(rdt_of$`rdt solid waste`)
check_outliers(rdt_of$`rdt liquid waste`)
...
# 
lmtest::bptest(rdt_of$`rdt food waste`)
lmtest::bptest(rdt_of$`rdt solid waste`)
lmtest::bptest(rdt_of$`rdt liquid waste`)
...
# 5.2 check linearity
check_heteroscedasticity(rdt_of$`rdt food waste`)
check_heteroscedasticity(rdt_of$`rdt solid waste`)
check_heteroscedasticity(rdt_of$`rdt liquid waste`)

# 6. No multicollinearity
ass_multi_sfw_log[[5]]  + labs(title = "VIF", subtitle = "")
#6.1 check collinearity
coll_fw  <- check_collinearity(rdt_of$`rdt food waste`)
coll_fw
```
```{r eval = FALSE, echo = FALSE}
