...
df <- df %>%
  ...
  mutate(time = seq(1:(nrow(df) - sum(is_closed))))
cutoff <-  df %>% filter(date %in% as.Date('2023-01-03')) %>%
                    select(time) %>% as.numeric()
df %>% mutate(time = time - cutoff)

# simple food waste per customer -----
rdt_fwp <- food_waste_p_kg ~ container * time 

summary(rdt_fwp <- lm(formula =  rdt_fwp, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_fwp)

# simple solid food waste per customer -----
rdt_sfwp <- solid_waste_p_kg ~ container * time 

summary(rdt_sfwp <- lm(formula =  rdt_sfwp, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_sfwp)
# simple liquid food waste per customer -----
rdt_lfwp <- liquid_waste_p_kg ~ container * time 

summary(rdt_lfwp <- lm(formula =  rdt_lfwp, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_lfwp)

# multi food waste per customer -----
rdt_fwp_multi <- food_waste_p_kg ~ container * time +
                            temp_c + humi_p + prcp_mm + 
                            liquors + sales + halfs

summary(rdt_fwp_multi <- lm(formula =  rdt_fwp_multi, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_fwp)

# multi solid food waste per customer -----
rdt_sfwp_multi <- solid_waste_p_kg ~ container * time +
                            temp_c + humi_p + prcp_mm + 
                            liquors + sales + halfs

summary(rdt_sfwp_multi <- lm(formula =  rdt_sfwp_multi, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_sfwp)
# multi liquid food waste per customer -----
rdt_lfwp_multi <- liquid_waste_p_kg ~ container * time +
                            temp_c + humi_p + prcp_mm + 
                            liquors + sales + halfs

summary(rdt_lfwp_multi <- lm(formula =  rdt_lfwp_multi, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_lfwp)

# poly- food waste per customer -----
rdt_fwp_poly <- food_waste_p_kg ~ container * time + 
                             container * I(time^2) +
                             temp_c + humi_p + prcp_mm + 
                             liquors + sales + halfs

summary(rdt_fwp_poly <- lm(formula =  rdt_fwp_poly, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_fwp)

# poly- solid food waste per customer -----
rdt_sfwp_poly <- solid_waste_p_kg ~ container * time + 
                               container * I(time^2) +
                               temp_c + humi_p + prcp_mm + 
                               liquors + sales + halfs

summary(rdt_sfwp_poly <- lm(formula =  rdt_sfwp_poly, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_sfwp)
# poly- liquid food waste per customer -----
rdt_lfwp_poly <- liquid_waste_p_kg ~ container * time + 
                                container * I(time^2) +
                                temp_c + humi_p + prcp_mm + 
                                liquors + sales + halfs

summary(rdt_lfwp_poly <- lm(formula =  rdt_lfwp_poly, 
                       data = df, subset = (!df$is_closed)))
# tidy(rdt_lfwp)
```
...
1. plot residual
2. X robustness checks (overfitting check)
3. X placebo tests
